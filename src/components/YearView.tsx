...
const YearView: React.FC<YearViewProps> = ({ events, currentDate, setCurrentDate, onEventClick, onDateClick, onShowMore }) => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4 overflow-y-auto">
      {months.map((month) => (
        <motion.div
          key={month.getMonth()}
         // whileHover={{ scale: 1.02 }}
          onClick={() => setCurrentDate(month)}
          className="border border-gray-200 rounded-md hover:shadow-md transition cursor-pointer min-h-[150px] sm:min-h-[250px]"
        >
          <h3 className="text-center font-semibold py-1 sm:py-2 bg-gray-50 border-b border-gray-200 text-sm sm:text-base">
